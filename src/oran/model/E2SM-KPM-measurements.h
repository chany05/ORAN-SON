...
const std::map<std::string, std::tuple<enum MeasurementType, std::string, enum FeatureImplemented>>
    m_defaultEndpointsKpmTypeAndUnit = {
        ...
        {"ERAB.UsageNbrMean.QCI",
         {INTEGER, "", NOTIMPLEMENTED}}, // mean simultaneous e-RABs in a period
        ...
        {"HO.IntraEnbOutAtt.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // RRCConnectionReconfigurationMessage added to handover
        {"HO.IntraEnbOutSucc.Cause",
         {INTEGER,
          "",
          NOTIMPLEMENTED}}, // RRCConnectionReconfigurationCompleteMessage added to handover
        {"HO.IntraDenbOutPrepToRnAtt.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // X2AP HANDOVER_REQUEST added to handover
        {"HO.IntraDenbOutToRnAtt.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // RRCConnectionReconfigurationMessage added to handover
        {"HO.IntraDenbOutToRnSucc.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // X2AP UE_CONTEXT_RELEASE added to thandover
                                         // 4.3.1.2 Inter-eNB Handover Related Measurements
        {"HO.InterEnbOutPrepAtt",
         {INTEGER, "", NOTIMPLEMENTED}}, // X2AP HANDOVER_REQUEST + S1AP HANDOVER_REQUIRED
        {"HO.InterEnbOutAtt.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // RRC ConnectionConfigurationMessage
        {"HO.InterEnbOutSucc.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // X2AP UE_CONTEXT_RELEASE
                                         // 4.3.1.3 Handover Measurements On Neighbour Cell Basis
        {"HO.OutAttTarget.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // X2AP HANDOVER_REQUEST + S1AP HANDOVER_REQUIRED
        {"HO.OutSuccTarget.Cause",
         {INTEGER, "", NOTIMPLEMENTED}}, // RRC ConnectionConfigurationMessage
        ...
        {"DRB.PdcpSduAirLossRateUl.QCI",
         {INTEGER, "", NOTIMPLEMENTED}}, // the standard doesn't use Air for UL, which is weird
        ...
        {"PAG.DiscardedNbr",
         {INTEGER, "", NOTIMPLEMENTED}}, // X == (T/N)*(UE_RNTI % N) && Y == floor(UE_RNTI/N) % N &&
                                         // max_paging_records_queue.isFull()
         ...
         * MeasurementReport messages are excluded. This measurement shall be increased for each
         * reported value RSRP_LEV (See in 3GPP TS 36.331 [18]). For every one or two or 17 or 20
         * RSRP_LEV(s) a separate measurement is defined. (See in 3GPP TS 36.133 [19]) d) Each
         * measurement is an integer value. e) MR.Rsrp.y . y is an integer from 00 to 47. Note: 00
         * of y indicates from RSRP _LEV _00 to RSRP _LEV _LEV _20, namely RSRP < -120dBm, 01 of y
         * indicates RSRP _LEV _21 and RSRP _LEV _25, namely -120 ≤ RSRP < -115dBm, 02 of y
         * indicates RSRP _LEV _26, namely -115≤ RSRP < -114dBm, 03 of y indicates RSRP _LEV _27,
         * namely -114≤ RSRP < -113dBm … 36 of y indicates RSRP _LEV _60, namely -81 ≤ RSRP <
         * -80dBm, 37 of y indicates RSRP _LEV _61, RSRP _LEV _62, namely -80 ≤ RSRP < -78dBm, 38 of
         * y indicates RSRP _LEV _63, RSRP _LEV _64, namely -78 ≤ RSRP < -76dBm, … 46 of y indicates
         * from RSRP _LEV _79 to RSRP _LEV _80, namely -62 ≤ RSRP < -60dBm, 47 of y indicates from
         * RSRP _LEV _81 to RSRP _LEV _97, namely -60dBm ≤ RSRP. (See in 3GPP TS36.133 [19])
         */
        // 6.2 RSRQ Related Measurements
        /**
         * Receipt by the eNodeB from the UE of MeasurementReport message indicating a periodical UE
         * measurement report where IE MeasResults field includes rsrqResult. The event triggered
         * MeasurementReport messages are excluded. This measurement shall be increased for each
         * reported value RSRQ_LEV (See in 3GPP TS 36.331 [18]). For every one or two RSRQ_LEV(s) a
         * separate measurement is defined. (See in 3GPP TS 36.133 [19]) d) Each measurement is an
         * integer value. e) MR. Rsrq.y y is an integer from 00 to 17. Note: 00 of y indicates
         * RSRQ_00, namely RSRQ < -19.5dB, 01 of y indicates RSRQ_01 and RSRQ_02, namely -19.5 ≤
         * RSRQ < -18.5dB, 02 of y indicates RSRQ_03 and RSRQ_04, namely -18.5 ≤ RSRQ < -17.5dB, …
        ...
        /**
         * c) Receipt by the eNodeB from the UE of Power Headroom Report message indicating a
         * periodical UE measurement POWER_HEADROOM report. This measurement shall be increased for
         * each reported value POWER_HEADROOM (See in 3GPP TS 36.321 [16]). For every one
         * POWER_HEADROOM (s) a separate measurement is defined. (See in 3GPP TS 36.133 [19]). d)
         * Each measurement is an integer value. e) MR.PowerHeadRoom.y . y is an integer from 00
         * to 63. Note: 00 of y indicates POWER_HEADROOM_00, namely -23 ≤ PH < -22, 01 of y
         * indicates POWER_HEADROOM_01, namely -22 ≤ PH < -21, … 63 of y indicates
         * POWER_HEADROOM_63, namely PH≥40. (See in 3GPP TS36.133 [19])
         */
        // 6.4 UE Rx-Tx Time Difference Related Measurements
        /**
         * Receipt by the eNodeB from the UE of MeasurementReport message indicating a periodical UE
         * measurement report where IE MeasResults field includes ue-RxTxTimeDiffResult. The event
         * triggered MeasurementReport messages are excluded. This measurement shall be increased
         * for each reported value RXTX_TIME_DIFFERENCE (See in 3GPP TS 36.331 [18]). For every 5 or
         * 20 or 100 or 524 RXTX_TIME_DIFFERENCEs a separate measurement is defined. (See in 3GPP
         * TS 36.133 [19]) d) Each measurement is an integer value. e) MR.RxTxTimeDiff.y y is an
         * integer from 00 to 47 Note: 00 of y indicates from RX-TX_TIME_DIFFERENCE_0000 to
         * RX-TX_TIME_DIFFERENCE_0004, namely 0 Ts ≤ TUE Rx-Tx< 10 Ts, … 19 of y indicates from
         * RX-TX_TIME_DIFFERENCE_0095 to RX-TX_TIME_DIFFERENCE_0099, namely 190Ts ≤ TUE RX-TX< 200
         * Ts, 20 of y indicates from RX-TX_TIME_DIFFERENCE_0100 to RX-TX_TIME_DIFFERENCE_0119,
         * namely 200Ts ≤ TUE RX-TX< 240 Ts, … 39 of y indicates from RX-TX_TIME_DIFFERENCE_0480 to
         * RX-TX_TIME_DIFFERENCE_0499, namely 960Ts ≤ TUE RX-TX< 1000 Ts, 40 of y indicates from
         * RX-TX_TIME_DIFFERENCE_0500 to RX-TX_TIME_DIFFERENCE_0599, namely 1000Ts ≤ TUE RX-TX< 1200
         * Ts, 44 of y indicates from RX-TX_TIME_DIFFERENCE_0900 to RX-TX_TIME_DIFFERENCE_0999,
         * namely 1800Ts ≤ TUE RX-TX< 2000 Ts, 45 of y indicates from RX-TX_TIME_DIFFERENCE_1000 to
         * RX-TX_TIME_DIFFERENCE_1523, namely 2000Ts ≤ TUE RX-TX< 3048 Ts, 46 of y indicates from
         * RX-TX_TIME_DIFFERENCE_1524 to RX-TX_TIME_DIFFERENCE_2047, namely 3048 Ts ≤ TUE RX-TX<
         * 4096 Ts, 47 of y indicates from RX-TX_TIME_DIFFERENCE_2048 to RX-TX_TIME_DIFFERENCE_4095,
         * namely 4096 Ts ≤ TUE RX-TX (See in 3GPP TS36.133[20])
         */
        // 6.5 AOA Related Measurements
        /**
         * c) Receipt by the eNodeB from the UE of MeasurementReport message indicating a periodical
         * UE measurement report where IE MeasResults field includes aoaResult. The event triggered
         * MeasurementReport messages are excluded. This measurement shall be increased for each
         * reported value AOA_ANGLE (See in 3GPP TS 36.331 [18]). For every 5 AOA_ANGLE (s) a
         * separate measurement is defined. (See in 3GPP TS 36.133 [19]) Note: antenna deployment.
